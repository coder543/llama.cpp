...
test('reasoning + tool + final content stream into one reasoning block', async ({ page }) => {
	...
	// Assert via exposed debug data that merged thinking includes the second step
	const debugMessages = await page.evaluate(() => (window as any).__debugDisplayMessages);
	expect(
		debugMessages?.find((m: any) => m.message?.role === 'assistant')?.message?.thinking ?? ''
