...
test('reasoning -> tool -> reasoning streams inline without refresh', async ({ page }) => {
	...
	await page.addInitScript(() => {
		...
		(window as any).__completionCallCount = 0;
		...
		window.fetch = (input: RequestInfo | URL, init?: RequestInit) => {
			...
			if (url.includes('/v1/chat/completions')) {
				completionCall += 1;
				(window as any).__completionCallCount = completionCall;
				...
				if (completionCall === 2) {
					...
					(window as any).__flushSecondStream = () => {
	...
	// Wait for first reasoning chunk to appear (store + UI)
	await expect
		.poll(() =>
			page.evaluate(
				() =>
					(window as any).__debugDisplayMessages?.[1]?.message?.thinking ??
					(window as any).__debugActiveMessages?.[1]?.thinking ??
					''
			)
		)
	...
	await expect
		.poll(() => page.evaluate(() => (window as any).__completionCallCount || 0))
		.toBeGreaterThanOrEqual(2);

	// Capture debug state to aid diagnosing reactivity issues
	const debugState = await page.evaluate(() => {
		const simplify = (m: any) =>
			m
				? {
						id: m.id,
						role: m.role,
		...
		const active = ((window as any).__debugActiveMessages || []).map(simplify);
		const display = ((window as any).__debugDisplayMessages || []).map((d: any) => ({
			id: d?.message?.id,
			thinking: d?.message?.thinking,
			content: d?.message?.content,
			segments: d?.message?._segments
		}));
		return { active, display };
	});
	// eslint-disable-next-line no-console
	console.log('DEBUG_STATE', JSON.stringify(debugState, null, 2));
	// eslint-disable-next-line no-console
	console.log(
		'REASONING_TEXT',
		await page.locator('[aria-label=\"Assistant message with actions\"]').first().innerText()
	);
	...
	await page.evaluate(() => (window as any).__flushSecondStream?.());
